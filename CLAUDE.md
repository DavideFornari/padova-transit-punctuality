...
## Task 1 — fct timestamps depend on the machine's timezone  [CORRECTNESS, HIGH]

**File:** `dbt/models/marts/fct_stop_events.sql`

**Problem (verified):** `(x::timestamp)::timestamptz` interprets the naive timestamp
in the DuckDB *session* timezone. The same input row produces `scheduled_arrival`
08:00 on a Rome-timezone machine, 10:00 in Docker (UTC), 14:00 in New York. The
dashboard's "by hour of day" chart buckets on this column, so it shifts with the
machine that ran dbt. GTFS times are agency-local (Europe/Rome) wall-clock by
definition — no conversion is needed at all.
...
**Edit 4 —** in `dbt/models/marts/_marts.yml`, in the `fct_stop_events` description,
change `(Europe/Rome)` to `(Europe/Rome wall clock, naive timestamps)`.

**Test to add** in `tests/test_dbt_models.py` (copy the import/fixture style of the
