...
Creates a minimal set of Parquet files mimicking the ingestion output,
runs dbt, and queries the resulting DuckDB warehouse to verify the
fact table correctly joins RT data to the right schedule version and
handles midnight-crossing times.
...
# Skip the entire module if dbt is not installed
...
def _create_synthetic_data(data_dir: Path) -> None:
    ...
    # stop_times — include a midnight-crossing time (25:15:00)
    pq.write_table(
        pa.table(
            {
    ...
    # ── RT vehicle positions (minimal, for staging test only) ────────────
...
def test_dbt_run_succeeds(dbt_warehouse: str) -> None:
    # If we get here, the fixture already asserted success.
